...
**Resolved (2026-08-02) — report hub grouped by access need (the ordering §13 flagged twice):**
- ✅ **The problem:** the catalog reached 18 facts (0013 + 0015) and the report hub listed every applicable one in attribute-**key** order — an implementation detail leaking into the interface. For a provider that put "Wheelchair-accessible scale" third, between the restroom and the captions, for no reason a reader could infer. At 17 rows an arbitrary order stops being untidy and becomes unusable: someone who just visited had to read the whole list to find the two things they could speak to.
- ✅ **Grouped by the access need each fact speaks to** (`src/lib/attribute-groups.ts`, pure + unit-tested), applied to BOTH hub lists so tracked and not-yet-reported facts can't drift apart: Wheelchair and mobility / Blind and low-vision / Deaf and hard-of-hearing / Cognitive and learning / Sensory and neurodivergent / Everyone.
- ✅ **Headings name a SUBJECT, never a permission.** "Wheelchair and mobility access", never "If you use a wheelchair". Anyone may report any fact — the optional tag only *weights* a report (§4). A heading that read as a gate would suppress exactly the reports the consensus model wants, and would misdescribe how the formula works. A test asserts no heading starts with "if you" or contains "only", and the intro line says it in plain words.
- ✅ **Group order is the fixed `IDENTITY_TAGS` order, shared group last — deliberately NOT a ranking.** It's the same sequence the visit-report form already shows the tags in, so a contributor meets them twice in the same order. Alphabetical within a group: no implied priority between two facts serving the same need. (The ask-ahead page keeps its *urgency* ordering — disputed → stale → … — which is a different and still-correct job; only the hub was arbitrary.)
- ✅ **Nothing can fall out.** An attribute weighting a tag no longer offered lands in the shared group rather than vanishing — a fact must never become unreportable because the tag vocabulary moved underneath it. Tested, along with "the real catalog groups without loss" and "still produces ≥5 groups" (so a future change can't silently collapse it back to one list).
- ✅ **Required wiring:** `relevantIdentityTag` now flows through `AttributeDefOption` and `AttributeStatus` (it was already selected in one reader and dropped in the map), plus both seed mirrors — a mirror that dropped it would silently render one shared group.
- ✅ **Verified:** 185 unit + 111 a11y tests pass (axe covers the hub, so the new `h2` → `h3` nesting is checked). Rendered against local Postgres: all 18 provider facts present across 6 groups, in tag order with "Everyone" last.

**Resolved (2026-08-02) — the coverage write flow (ops CLI; migration `0016`):**
- ✅ **Ops writes it, and that is forced, not lazy.** Coverage's authority IS the practice, so a provider-facing form is the eventual answer — it is not buildable today and shipping it early would be dangerous: the Keycloak BFF is config-gated OFF and blocked on the IdP standup, and there is **no claim-a-listing / practice-verification flow at all**. Without proof that someone represents a practice, a public write on **money facts is a fraud vector** — mark a competitor "not accepting new patients" and you have diverted their patients. Every other write surface here is safe to open early because §4 consensus absorbs a bad actor (≥3 confirmations, dissent freezes); **coverage has no consensus layer by design, so one write publishes immediately.** `src/lib/coverage-write.ts` is the seam: a verified provider flow later replaces the INPUT without touching storage, presentation, or the fail-safe rules.
- ✅ **`npm run coverage:update`** — `--new-patients/--medicaid/--medicare/--telehealth yes|no|unknown`, `--source`, `--as-of` (defaults to today), `--note`, `--reason`, `--dry-run`, typed-word confirm. Same conventions as the other ops CLIs, reusing the one typed implementation.
- ✅ **Never store what we could not publish.** `coverage.ts` already refuses to RENDER an unsourced/undated fact; the write path now enforces the same rules **at the door**, because a database of unpublishable rows looks like data and reports like data. Rejects: no reason, no flags, no/invalid/**future** date, `sourced` without a citation (§7), a bad source vocabulary, a listing that is a place. Reports every problem at once — an operator on the phone should not re-run four times.
- ✅ **The retraction path stays open.** Clearing every flag needs no source or date (demanding provenance for a fact being *removed* would block "our note was wrong"), and clearing all of them clears the shared source/date/note too rather than leaving orphaned provenance describing nothing.
- 🐛 **A real bug, found by verifying against Postgres rather than by reasoning.** The first successful run updated only `--medicare` with a new `sourced` note — and **relabelled the other three flags**, publishing the practice's own self-attested panel status as "From: NY State of Health directory". Provenance is shared across the record (0014), so a partial write with new provenance asserts a source nobody has: **migration 0012's bug, reappearing in a new place.** Fixed by `carriedOverFlags`, which refuses any write that changes the shared source/date while leaving already-published flags un-restated, and names them. A partial add under the *same* provenance (same call, same batch) stays frictionless; an explicit `unknown` counts as re-stating.
- ✅ **Audited.** `moderation_audit` gains a `coverage_update` action rather than a parallel table — the shape is already right (append-only, ops-only, actor + reason mandatory, no `contributor_id`, content ids as plain uuids). Migration 0016 **widens the table's documented purpose to "operator actions on published data" and says so in the table comment**, rather than sneaking authoring in under a moderation label. `reason` carries how the operator knows ("called the office, spoke to reception").
- ✅ **Verified against real Postgres:** `--dry-run` is a true no-op; every refusal path writes nothing and no audit row; the relabelling write is refused with the seed row untouched; the three legitimate paths (same-provenance partial add, new-provenance full re-statement, full retraction) all work and record before/after; the trail stays append-only (`service_role` UPDATE/DELETE denied) and unreadable to `anon`. 205 unit + 113 a11y tests pass.
- ⏳ **Open:** no BULK path — `seed-import.mjs` doesn't carry coverage, and the realistic workflow is a calling campaign producing a spreadsheet. Per-flag provenance (rather than one shared source/date) would remove the `carriedOverFlags` friction entirely and is the better long-term shape. The provider-facing flow still needs the IdP standup **and** a practice-verification design.

**Resolved (2026-08-03) — bulk coverage: the call-sheet loop:**
- ✅ **The workflow, not just an importer.** A calling campaign produces a **spreadsheet**, so this works in CSV and round-trips the same file: `npm run coverage:sheet -- --out calls.csv` exports every provider pre-filled with what we hold (**ranked: nothing-recorded first, then stale, then current** — the call list, in order), the operator fills it in, `npm run coverage:import -- calls.csv` reads it back. The export exists because the import is unusable without it: rows are keyed by listing UUID and nobody looks up forty of those by hand.
- ✅ **BLANK MEANS LEAVE UNCHANGED, never clear.** The most consequential decision here. A call sheet is always partly filled — you reached twelve of forty practices, and you didn't ask every practice every question. If blank meant "set to unknown", one half-finished sheet would silently wipe every fact it didn't mention. To clear a fact you type `unknown`, which is a decision someone made rather than the absence of one. A row with every flag blank is skipped **quietly** — an unreached practice is the normal state of a working sheet, not an error.
- ✅ **Bulk gets no weaker standard.** Every row runs the SAME `validateCoverageWrite` + `carriedOverFlags` rules as the single-record CLI — bulk is exactly where a weaker rule would do the most damage. Because blank means leave-unchanged, the relabelling trap re-opens per row, and it is rejected there rather than given a second, looser rule.
- ✅ **The whole batch is validated before anything is written** (seed-import's posture). A forty-row import that half-applies is worse than one that refuses. Honest caveat, stated in the script: supabase-js exposes no multi-statement transaction, so a failure *during* apply leaves earlier rows written — which is why validation is exhaustive, why each applied row is audited, and why the failure message says exactly how many landed and that re-running is harmless.
- ✅ **Row-shift guard.** The `name` column is checked against the database. Someone inserting a row in a spreadsheet and sliding the ids out of alignment is a realistic disaster — it would write each practice's answers onto its neighbour. A name mismatch refuses the batch. Duplicate `listing_id`s are refused too, rather than letting the last row silently win.
- ✅ **Hand-written CSV, because it had to be.** No CSV dependency exists and this is the only place that needs one — but a spreadsheet WILL emit a quoted comma in a note ("NY State of Health, Aug 2026"), so quoting is not optional. RFC 4180 subset: quoted fields, doubled quotes, commas/newlines inside quotes, CRLF, trailing newline, **and the UTF-8 BOM Excel writes** (without which the first header becomes `﻿listing_id` and no column matches). 20 unit tests, mostly on the parser.
- ✅ **Verified end-to-end against real Postgres:** ranked export; a messy sheet refused for BOTH a row-shift and a relabelling attempt with **zero rows written** (including the valid row — all-or-nothing); the corrected sheet applying two rows and quietly skipping the unreached one; one audit entry per applied record carrying the operator's own words; the quoted-comma note surviving the write and the re-export. 225 unit + 113 a11y tests pass.
- ⏳ **Open:** per-flag provenance would remove the `carriedOverFlags` friction that blank-means-unchanged keeps re-opening — it is now the clearest next refactor. A provider-facing flow still needs the IdP standup **and** a practice-verification design.

**Resolved (2026-08-03) — per-flag coverage provenance (migration `0017`): the refactor that DELETED a guard:**
- ✅ **The problem, which showed up twice.** 0014 stored ONE `coverage_source` / `coverage_as_of` / `coverage_note` for the whole provider — right when written in one go, wrong the moment anything updates part of it. The single-record write path (0016) hit it as a **bug** caught only against real Postgres: updating just `--medicare` from a directory relabelled the practice's own self-attested panel status as "From: NY State of Health directory". The bulk call sheet hit it again as **friction**: blank means leave-unchanged, so `carriedOverFlags` had to reject rows and make operators re-state facts they never asked about. **Twice is a wrong model, not two coincidences** — the guard was protecting us from our own schema.
- ✅ **One row per (provider, fact)**, each carrying its own `source` / `as_of` / `note` — the same shape as `attribute_claims`, and for the same reason: a fact is a thing with its own evidence, not a column on a form. Fact keys are **text against an app-side vocabulary**, not an enum, so a future coverage fact is a data change (as `listings.category` chose in 0005).
- ✅ **The real prize: `source` and `as_of` are NOT NULL, so a fact that cannot be published cannot be stored.** The fail-safe moved out of runtime code and into the schema, where it can't be forgotten. `presentCoverage`'s four-branch refusal collapsed to one structural check (corrupt/future date — degrade to silence, not to a claim). **Unknown is now the absence of a row**: no three-valued booleans, no "set but unsourced" state.
- ✅ **`carriedOverFlags` is deleted, and so are its six tests.** A partial write is now simply safe — which is what it always looked like. The unit suite went **225 → 219**: fewer tests because there is less to guard. The bulk importer lost its per-row provenance check entirely; blank finally means blank.
- ✅ **Lossy backfill, on purpose.** A flag with no source or no date was never renderable (0014's presenter refused it), so it was invisible data — carrying it over would mean inventing provenance it never had. Verified against real old-shape rows: a publishable practice migrated 3 facts (`false` preserved as false, the NULL flag skipped), and the unsourced and undated practices migrated **zero**. Old columns dropped so the two shapes can't drift.
- ✅ **Verified end-to-end against Postgres:** the exact write 0016 had to REFUSE now succeeds correctly — Medicare takes the directory citation and today's date while the other three keep "the practice told us this themselves" and their original date; clearing one fact deletes only its row; a batch row that #44 would have rejected now just works; `NOT NULL` rejects an unsourced insert; the provider page, all three filters (`?medicare=1` still correctly empty), and the ask-ahead "what we hold" lines all render from the new shape. 219 unit + 113 a11y pass.
- ⏳ **Open:** the provider-facing flow still needs the IdP standup **and** a practice-verification design. The call sheet exports one `source`/`note` column pair, so a sheet still applies one provenance per row — per-fact provenance *in the sheet* is possible now but would make it much harder to fill in by hand; deliberately not done.

**Resolved (2026-08-03) — an attribute may weight SEVERAL access experiences (migration `0018`):**
- ⚠️ **This is the one change in this run that touches the §4 consensus formula.** The formula lives in TWO places (`attribute_claim_status` and the `src/lib/seed.ts` mirror) and both moved together, as §4/§13 require.
- ✅ **The gap (open since 0013):** `relevant_identity_tag` was ONE text column, so an attribute weighted exactly one tag. `service_animal_welcomed` was the clearest casualty — guide dogs, hearing dogs and mobility assistance dogs are all service animals and every handler is a first-person authority on whether the animal was welcomed, but only `blind_low_vision` counted. A Deaf handler's report of being refused with a hearing dog did not weight the fact it was most qualified to speak to.
- ✅ **Deliberately TWO parts, and only the second changes behavior.** **Part 1 is structural and behavior-preserving:** scalar → `text[]`, and `tag = any(reviewer_tags)` → the overlap `tags && reviewer_tags`. For a ONE-element array those are exactly equivalent, so converting every existing attribute to a single-element array leaves every state and weighted count bit-for-bit unchanged — that equivalence is what makes the refactor reviewable. **Part 2 is one deliberate data change:** widening a tag set *loosens* that attribute's verification bar, so exactly one attribute was widened and it is argued in the migration.
- ✅ **`service_animal_welcomed` → `{blind_low_vision, deaf_hoh, wheelchair_user}`.** **NOT** `cognitive_access` / `neurodivergent`: psychiatric service dogs are real and their handlers equally authoritative, but neither coarse tag *means* "psychiatric disability", and stretching one would mislabel a group rather than include it — the same reasoning that leaves `seating_available` and `telehealth_audio_only` weighting nobody. Those handlers can still report; the report simply isn't weighted, which is honest until the tag vocabulary is community-reviewed.
- ✅ **Empty array, not NULL, for "no privileged identity"** — `&&` against NULL is NULL rather than false, which would silently break the count. `not null default '{}'` plus `cardinality(...) = 0` removes the ambiguity.
- ✅ **The report hub lists a multi-tag fact under EVERY need it serves.** "Service animal welcomed" now appears under wheelchair, blind/low-vision AND Deaf/HoH. Appearing three times is the point: the groups answer "what can I speak to", and for that fact several people can.
- ✅ **Verified against real Postgres, in the order that matters.** Captured all five claim states BEFORE the migration, applied it, and confirmed the output is **identical** — behavior preservation proved, not assumed. Then the new behavior, on a constructed case: three agreeing visits where the only tagged reporter is a Deaf hearing-dog handler now gives `weighted_agree_count = 1 → community_verified`, where the old single-tag rule gave `weighted = 0 → community_confirmations`. All three view grants (anon, authenticated, service_role) survived the required `drop view` — **dropping a view drops its grants**, and 0009's service_role grant is easy to lose here. 222 unit + 113 a11y pass.
- ✅ **Guarded against creep:** a test asserts `service_animal_welcomed` is the ONLY multi-tag attribute. Widening loosens a safety bar, so each future one must be argued in its migration and added to that list deliberately.
- 🐛 **Caught in verification:** the first draft dropped the scalar column before dropping the dependent view, which Postgres refuses. Ordering fixed; the migration is now drop-view → add array → backfill → drop scalar → recreate view → re-grant.

**Resolved (2026-08-03) — the coverage staleness window is now PER FACT:**
- ✅ **The honest answer to "is 180 right?" was not a different number.** Nobody has watched real coverage data decay, so swapping one guess for another would be theatre. The actual defect was that **one window covered four facts that decay at genuinely different rates**, and it was wrong in both directions at once.
- ✅ **`COVERAGE_STALE_DAYS` is now a per-key map:** `acceptingNewPatients` **90** (a panel can close in a month — the fastest-moving and most trip-wasting fact); `acceptsMedicaid` / `acceptsMedicare` **180** (contract cycle; plan years turn over annually); `offersTelehealth` **365** (a service line, not a cycle — a practice offering telehealth in March is almost certainly still offering it in December).
- ✅ **The safety-relevant direction is the tightening.** A four-month-old "accepting new patients" previously published with **no warning at all**; it now warns. Verified end-to-end: a 120-day panel status renders "over three months old — check before you travel" while the same provider's 28-day Medicaid and telehealth facts stay clean.
- ✅ **Loosening telehealth is deliberate, not a slip.** Warning "this is old" about a stable service line trains people to ignore the warning where it matters. A staleness notice that fires on everything is worth nothing.
- 🐛 **Fixed a latent copy bug found on the way:** "over six months old" was **hardcoded in two separate files** (`providers/[id].astro` and `call-ahead.ts`), so any change to the number would have silently made both sentences lie. The phrase is now DERIVED from the fact's own window (`stalenessWindowPhrase`, surfaced as `staleAfter` on the presentation), and a test pins it.
- ✅ **The call sheet ranks per fact too.** `isCoverageStale(key, asOf)` is shared by the presenter and the ranking, so a provider whose panel status is four months old ranks as needing a call even though its Medicaid answer is still fresh — previously one cutoff date judged the whole record.
- ✅ **Why TS and not the DB:** the attribute side keeps `reverify_interval_days` per row because attributes are extensible data. Coverage fact keys are a FIXED app-side vocabulary (0017), and this is presentation policy that will be tuned — so it lives in code, guarded by a test that the map covers exactly `COVERAGE_ORDER` (a new key can't arrive with `undefined` days and silently never go stale) and that no window exceeds the 365-day attribute cadence.
- ✅ **Verified:** 225 unit + 113 a11y pass. Against real Postgres + the dev server: the 120-day panel status is stale under the new window (and was not under the old), the derived phrase renders on both the provider page and the ask-ahead "what we hold" line, and the call sheet ranks the provider as stale off that one fact.
- ⏳ **Still a judgement, not evidence.** These four numbers should be tuned once a calling campaign has produced real history — that is the one thing that would replace reasoning with data.
